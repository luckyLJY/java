 ...
 * Package: com.macro.mall.tiny.service.impl
 * Version: 1.0
 * Created by ljy on 2022-4-25 14:44
 */
...
public class UmsPermissionServiceImpl implements UmsPermissionService {
    ...
    public void streamTest() {
        ...
        long dirPermissionCount = permissionList.stream()
                .filter(permission -> permission.getType() == 0)
                .count();
        LOGGER.info("cont操作:{}",dirPermissionCount);
        ...
        //collect装map操作:将权限列表以id为key,以权限对象为值转换成map
    ...
    private UmsPermissionNode covert(UmsPermission permission,List<UmsPermission> permissionList){
        ...
        List<UmsPermissionNode> children = permissionList.stream()
                .filter(subPermisson -> subPermisson.getPid().equals(permission.getId()))
