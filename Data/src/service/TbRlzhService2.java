...
public class TbRlzhService2 {
	
	
	//存储递归结果
	 List<FrHrOgr> tempList1 = new ArrayList<FrHrOgr>();
	 List<FrHrOgr> tempListAll=new ArrayList<FrHrOgr>();
	...
	//查询所有组织结构
	public   void getAllFrHrOgr() {
		FrHrOgrDao frhrogrdao = new FrHrOgrDao();
		tempListAll=frhrogrdao.getAll();
	}
	
	//在组织结构中通过对应对象的code查询组织对象
	public  FrHrOgr getFrHrOgrbyCode(String code) {
		FrHrOgr tempfrhrogr = new FrHrOgr();
		for(int i=0;i<tempListAll.size();i++) {
			FrHrOgr tempfrhrogr1 = new FrHrOgr();
			tempfrhrogr1=tempListAll.get(i);
			if(code.equals(tempfrhrogr1.getCode())) {
				tempfrhrogr=tempfrhrogr1;
			}
		}
		return tempfrhrogr;
	}
	
	//在组织结构中通过对应对象的fathcode查询组织对象list
		public  List<FrHrOgr> getFrHrOgrbyFathcode(String facthcode) {
			List<FrHrOgr> tempfrhrogrlist =new  ArrayList<FrHrOgr>();
			for(int i=0;i<tempListAll.size();i++) {
				FrHrOgr tempfrhrogr1 = new FrHrOgr();
				tempfrhrogr1=tempListAll.get(i);
				if(facthcode.equals(tempfrhrogr1.getFathcode())) {
					tempfrhrogrlist.add(tempfrhrogr1);
				}
			}
			return tempfrhrogrlist;
		}
	...
	public  void getFrHrOgr(FrHrOgr frhrogr) {
		...
		frhrogr1 = getFrHrOgrbyCode(frhrogr.getCode());
		//System.err.println(frhrogr1.toString());
		//查询本级组织的下级组织信息
		//tempList = FrHrOgrdao.getByFrHrOrgCode(frhrogr1.getCode());
		tempList = getFrHrOgrbyFathcode(frhrogr1.getCode());
		...
		for (int j = 0; j < frhrogrsList.size(); j++) {
			...
			//调用递归
			 ...
			 for(int i=0;i<tempList1.size();i++) {
				 ...
				 //System.out.println(tbhuanpersList.size()+"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"+tempList1.get(i).getName());
